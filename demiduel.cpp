...
//{
#define FIGHTER_POWER_CONDITION effect_count(POWER_EFFECT)
constexpr const char* FIGHTER_POWER_REPRESENTATION = "Power";
#define FIGHTER_POWER_VALUE std::to_string(effect_count(POWER_EFFECT))
...
//{
#define PLAYER_POWER_CONDITION effect_count(POWER_EFFECT)
constexpr const char* PLAYER_POWER_REPRESENTATION = "Power";
#define PLAYER_POWER_VALUE std::to_string(effect_count(POWER_EFFECT))
...
//{
#define FORSAKEN_CONDITION effect_count(FORSAKEN_EFFECT)
constexpr const char* FORSAKEN_REPRESENTATION = "Forsaken";
#define FORSAKEN_VALUE std::to_string(effect_count(FORSAKEN_EFFECT))
...
//{
#define CURSE_CONDITION effect_count(CURSE_EFFECT)
constexpr const char* CURSE_REPRESENTATION = "Curse";
#define CURSE_VALUE std::to_string(effect_count(CURSE_EFFECT))
...
constexpr int EXPLANATION_COUNT = 13;
...
#define EFFECT_CONDITIONS(X) (                            \
	...
	: X == 7 ? static_cast<bool>(CRIPPLE_CONDITION)       \
	: X == 8 ? static_cast<bool>(IMPAIR_CONDITION)        \
	: X == 9 ? static_cast<bool>(FIGHTER_POWER_CONDITION) \
	: X == 10 ? static_cast<bool>(AGILITY_CONDITION)      \
	: X == 11 ? static_cast<bool>(CURSE_CONDITION)        \
...
#define EFFECT_EXPLANATIONS(X) (         \
	...
	: X == 7 ? CRIPPLE_EXPLANATION       \
	: X == 8 ? IMPAIR_EXPLANATION        \
	: X == 9 ? FIGHTER_POWER_EXPLANATION \
	: X == 10 ? AGILITY_EXPLANATION      \
	: X == 11 ? CURSE_EXPLANATION        \
	: RANK_BOND_EXPLANATION              \
)
//}

// The total number of player-specific explainable effects.
constexpr int PLAYER_EXPLANATION_COUNT = 9;
...
#define PLAYER_EFFECT_CONDITIONS(X) (                      \
	...
	: X == 1 ? static_cast<bool>(FORBIDDEN_FUEL_CONDITION) \
	: X == 2 ? static_cast<bool>(BANISHMENT_CONDITION)     \
	: X == 3 ? static_cast<bool>(FREEDOM_CONDITION)        \
	: X == 4 ? static_cast<bool>(AGILITY_AURA_CONDITION)   \
	: X == 5 ? static_cast<bool>(HEAL_AURA_CONDITION)      \
	: X == 6 ? static_cast<bool>(PLAYER_POWER_CONDITION)   \
	: X == 7 ? static_cast<bool>(END_DRAW_CONDITION)       \
	: static_cast<bool>(END_DISCARD_CONDITION)             \
)
//}
...
#define PLAYER_EFFECT_VALUES(X) (   \
	...
	: X == 1 ? FORBIDDEN_FUEL_VALUE \
	: X == 2 ? BANISHMENT_VALUE     \
	: X == 3 ? FREEDOM_VALUE        \
	: X == 4 ? AGILITY_AURA_VALUE   \
	: X == 5 ? HEAL_AURA_VALUE      \
	: X == 6 ? PLAYER_POWER_VALUE   \
	: X == 7 ? END_DRAW_VALUE       \
	: END_DISCARD_VALUE             \
)
//}
...
#define PLAYER_EFFECT_EXPLANATIONS(X) (   \
	...
	: X == 1 ? FORBIDDEN_FUEL_EXPLANATION \
	: X == 2 ? BANISHMENT_EXPLANATION     \
	: X == 3 ? FREEDOM_EXPLANATION        \
	: X == 4 ? AGILITY_AURA_EXPLANATION   \
	: X == 5 ? HEAL_AURA_EXPLANATION      \
	: X == 6 ? PLAYER_POWER_EXPLANATION   \
	: X == 7 ? END_DRAW_EXPLANATION       \
...
constexpr int HOT_RODDER_ATTACK_DAMAGE = 450;
...
constexpr const char* DIRT_BIKER_ATTACK_DESCRIPTION =
	"Deal 400 damage to your opponent's active fighter. "
	"For each card in your trash, deal 5 more damage."
;
...
constexpr int ASTRONAUT_HEALTH = 1400;
...
constexpr const char* VACUUMANCER_ABILITY_DESCRIPTION =
	...
	"It can't retreat during your opponent's next turn."
;
...
constexpr int SCUBA_DIVER_ATTACK_DAMAGE = 500;
...
constexpr const char* PYROTECHNICIAN_ATTACK_DESCRIPTION =
	"Deal 0.2 damage to your opponent's active fighter "
...
constexpr int LOST_SOUL_HEALTH = 1200;
...
constexpr const char* LOST_SOUL_ABILITY_DESCRIPTION =
	...
	"its life card, and the top 3 cards of each player's deck."
;
...
constexpr const char* LOST_SOUL_ATTACK_DESCRIPTION =
	"If your deck has no fighter cards, deal 500 damage to your "
	"opponent's active fighter. Draw a fighter card from your deck."
;
...
constexpr int BANISHER_HEALTH = 1200;
...
constexpr const char* BANISHER_ATTACK_DESCRIPTION =
	"Your opponent's active fighter takes 25 damage at "
...
constexpr const char* BANSHEE_ATTACK_DESCRIPTION =
	...
	"deal 50 damage to your opponent's active fighter, "
	"for a maximum of 1000 damage."
;
...
constexpr int CULTIST_HEALTH = 1200;
...
constexpr const char* CULTIST_ABILITY_DESCRIPTION =
	"Once a turn, you may heal 20 damage from one "
...
constexpr const char* SENSEIS_CHOSEN_ATTACK_DESCRIPTION =
	"Deal 400 damage to your opponent's active fighter. "
	"Flip a coin. If heads, deal 100 more damage."
;
...
constexpr int NINJA_HEALTH = 1250;
...
constexpr const char* NINJA_ATTACK_DESCRIPTION =
	"Flip 3 coins. For each heads, deal 300 damage to "
	"one of your opponent's fighters for each heads."
;
...
constexpr int SAMURAI_RETREAT_COST = 2000;
...
constexpr const char* SAMURAI_ATTACK_DESCRIPTION =
	"Deal 450 damage to your opponent's active fighter. "
	"Flip 2 coins. If both of them are heads, the "
	"opponent's active fighter can't attack or "
...
constexpr int SAMURAI_ATTACK_DAMAGE = 450;
constexpr int SAMURAI_ATTACK_COST = 2000;
//}
//}
...
constexpr int BOXER_HEALTH = 1250;
...
constexpr const char* BOXER_ATTACK_DESCRIPTION =
	"Deal 1000 damage to your opponent's active fighter. "
	"Deal 100 less damage for each card in your hand."
;
...
constexpr int BOXER_ATTACK_DAMAGE = 1000;
...
constexpr const char* ELEMENTAL_ABILITY_DESCRIPTION =
	"If this fighter is your active fighter, you may defeat the fighters "
	"on your bench with this ability. If you defeat at least 3, rank this "
	"fighter up into an 'Omega Elemental' fighter card in your hand."
;
...
constexpr int FIRE_ELEMENTAL_HEALTH = 1000;
constexpr int FIRE_ELEMENTAL_RETREAT_COST = 1000;
constexpr const char* FIRE_ELEMENTAL_OLD_RANK = NO_OLD_RANK;
...
constexpr const char* FIRE_ELEMENTAL_ATTACK_NAME = "Singe";
constexpr const char* FIRE_ELEMENTAL_ATTACK_DESCRIPTION =
	"Deal 250 damage to your opponent's active fighter. "
	"Discard the top card of each player's deck."
;
const std::string FIRE_ELEMENTAL_ATTACK_EFFECTS(
	std::string(MILL_EFFECT) // mill
	+ EFFECT_SEPARATOR       //
	+ "1"                    // 1
	+ EFFECT_TERMINATOR
...
constexpr int FIRE_ELEMENTAL_ATTACK_DAMAGE = 250;
constexpr int FIRE_ELEMENTAL_ATTACK_COST = 1000;
//}
...
constexpr int AIR_ELEMENTAL_HEALTH = 1000;
constexpr int AIR_ELEMENTAL_RETREAT_COST = 1000;
constexpr const char* AIR_ELEMENTAL_OLD_RANK = NO_OLD_RANK;
...
constexpr const char* AIR_ELEMENTAL_ATTACK_DESCRIPTION =
	"Deal 250 damage to your opponent's active fighter. "
	"Both players draw a card."
;
const std::string AIR_ELEMENTAL_ATTACK_EFFECTS(
	std::string(DRAW_EFFECT) // draw
	+ EFFECT_SEPARATOR       //
	+ OPPONENT_EFFECT        // opponent
	+ EFFECT_SEPARATOR       //
...
constexpr int AIR_ELEMENTAL_ATTACK_DAMAGE = 250;
constexpr int AIR_ELEMENTAL_ATTACK_COST = 1000;
//}
...
constexpr int WATER_ELEMENTAL_HEALTH = 1000;
constexpr int WATER_ELEMENTAL_RETREAT_COST = 1000;
constexpr const char* WATER_ELEMENTAL_OLD_RANK = NO_OLD_RANK;
...
constexpr const char* WATER_ELEMENTAL_ATTACK_DESCRIPTION =
	"Heal 100 damage from all fighters. "
	"Deal 250 damage to your opponent's active fighter."
;
const std::string WATER_ELEMENTAL_ATTACK_EFFECTS(
	std::string(HEAL_EFFECT) // heal
	+ EFFECT_SEPARATOR       //
	+ OPPONENT_EFFECT        // opponent
	+ EFFECT_SEPARATOR       //
...
constexpr int WATER_ELEMENTAL_ATTACK_DAMAGE = 250;
constexpr int WATER_ELEMENTAL_ATTACK_COST = 1000;
//}
...
constexpr int EARTH_ELEMENTAL_HEALTH = 1000;
constexpr int EARTH_ELEMENTAL_RETREAT_COST = 1000;
constexpr const char* EARTH_ELEMENTAL_OLD_RANK = NO_OLD_RANK;
...
constexpr const char* EARTH_ELEMENTAL_ATTACK_NAME = "Tremor";
constexpr const char* EARTH_ELEMENTAL_ATTACK_DESCRIPTION =
	"Deal 100 damage to all fighters. "
	"Deal 250 damage to your opponent's active fighter."
;
const std::string EARTH_ELEMENTAL_ATTACK_EFFECTS(
	std::string(SPLASH_EFFECT) // splash
	+ EFFECT_SEPARATOR         //
	+ "100"                    // 100
	+ EFFECT_TERMINATOR
	...
	+ "100"                    // 100
	+ EFFECT_TERMINATOR
	+ RECOIL_EFFECT            // recoil
	+ EFFECT_SEPARATOR         //
	+ "100"                    // 100
);
constexpr int EARTH_ELEMENTAL_ATTACK_DAMAGE = 350;
constexpr int EARTH_ELEMENTAL_ATTACK_COST = 1000;
//}
...
constexpr const char* OMEGA_ELEMENTAL_ABILITY_DESCRIPTION =
	"Once a turn, you may search your opponent's trash for a card and draw it."
;
...
constexpr const char* OMEGA_ELEMENTAL_ATTACK_DESCRIPTION =
	"Deal 500 damage in total to your opponent's fighters. "
	"The damage is randomly distributed. "
	"Heal damage from this fighter equal to the damage dealt."
;
...
constexpr const char* INNKEEPER_DESCRIPTION =
	"Heal 250 damage from each of your fighters."
;
...
constexpr const char* DOCTOR_DESCRIPTION =
	"Discard your hand. "
	"Heal 600 damage from each of your fighters."
;
...
constexpr const char* PEACEMAKER_DESCRIPTION =
	...
	"deal 500 less damage until the start of your next turn."
;
...
constexpr int SUPPORTER_COUNT = 32;
...
class Player: public Affectable {
	public:
			...
			for (int i = 0; i < effects.size(); ++i) {
				...
				// Fusion summons a fighter from the hand if the conditions are met.
					...
					if (fusion_count >= std::stoi(effects[i][1])) {
						// The name of the fusion card.
						std::string name = ALL_FIGHTERS[std::stoi(effects[i][2])]->get_name();
		...
		void defeat_check() noexcept {
			...
			if (life_draws + life_banishes >= life_cards.size() && life_cards.size()) {
...
const DeckCode AGGRO_DECK(
	"Aggro",
	"An aggressive deck featuring Hot Rodder, Vacuumancer, Pyrotechnician, and Cloud Surfer.",
	{
		1, // DRIVER
		1, // RACER
		1, // HOT RODDER
...
const DeckCode TEMPO_DECK(
	"Tempo",
	"A offensive deck featuring Monster Trucker, Scuba Diver, Pyrotechnician, and Boxer.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode CONTROL_DECK(
	"Control",
	"A defensive deck featuring Astronaut, Cleric, Excavator, and Scuba Diver.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode SPLASH_DECK(
	"Splash",
	"A splash combo deck featuring Pirate, Warlock, Excavator, and Cloud Surfer.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode MILL_DECK(
	"Mill",
	"A mill deck featuring Pyromancer, Fire Elemental, and Air Elemental.",
	{
		0, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode VOID_DECK(
	"Void",
	"A Void deck featuring Lost Soul, Banisher, Banshee, and Cultist.",
	{
		0, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode COMBO_DECK(
	"Combo",
	"A Combo Attack deck featuring Lost Soul, Ninja, and Samurai.",
	{
		0, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode ELEMENTAL_DECK(
	"Elemental",
	"An elemental deck featuring Fire, Air, Water, Earth, and Omega Elemental.",
	{
		0, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
constexpr int DECK_CODE_COUNT = 9;
...
/* CHANGELOG:
	 ...
	 v1:
	   ...
	   Miller's draw count was decreased form 1 to 0.
