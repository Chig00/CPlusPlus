...
constexpr const char* HOT_RODDER_ATTACK_DESCRIPTION =
	"Deal 425 damage to your oppenent's active fighter."
;
...
constexpr const char* SUBSTITUTE_DESCRIPTION =
	"Switch in a fighter from your bench. Shuffle this card back "
...
;
constexpr const char* BOUNTY_HUNTER_EFFECTS = HOOK_EFFECT; // hook
//}
...
constexpr const char* SMUGGLER_DESCRIPTION =
	"Shuffle 5 cards from your trash into your deck."
;
...
constexpr const char* MILLER_DESCRIPTION =
	"Discard your hand. Discard the top card of your opponent's "
	"deck for each card discarded in this way."
;
...
constexpr const char* ALPHA_ENERGY_DESCRIPTION =
	...
	"When this energy card is attached to a fighter, draw a card."
;
...
constexpr const char* OMEGA_ENERGY_DESCRIPTION =
	...
	"When this energy card is attached to a fighter, "
	...
	public:
		...
		void play(Supporter& s) noexcept {
			...
			for (int i = 0; i < effects.size(); ++i) {
				...
				// The supporter card will go to the deck instead of the trash.
				...
				// Heals a fighter.
					...
					// Randomly distributes the healing for all fighters.
						...
						// A vector storing the maximum amount of healing
						//   each fighter can receive is initialised.
						...
						for (int i = 0; i < total_healing; ++i) {
							// The fighters are checked to see if they are all fully healed.
							int fully_healed = 0;
							
							for (int j = 0; j < fighters.size() && fully_healed; ++j) {
								if (healing[j] >= max_healing[j]) {
									++fully_healed;
								}
							}
							
							// If all of the fighters are fully healed, the healing stops.
							if (fully_healed == fighters.size()) {
								break;
							}
							
							// A uniform integral distribution for
							//   allocating the healing is intialised.
							// The fighter to be healed is determined.
							int pre_index = std::uniform_int_distribution<int>(
								0,
								fighters.size() - fully_healed - 1
							)(
								generator
							);
							
							// The index of the fighter to heal.
							int index;
							
							for (index = 0; index < fighters.size(); ++index) {
								// Only fighters that aren't fully healed are considered.
								if (healing[index] < max_healing[index]) {
									// The next fighter should be considered.
									if (pre_index) {
										--pre_index;
									}
									
									// This is the fighter to heal.
									else {
										break;
									}
								}
							...
							// The fighter's healing is incremented.
							++healing[index];
						}
						
						// The fighters are healed.
						for (int i = 0; i < fighters.size(); ++i) {
							fighters[i].heal(healing[i]);
				...
				// Returns the fighter and it previous ranks to hand.
					...
					while (true) {
						...
						// True if the previosu rank is found.
						bool found = false;
						...
						for (int i = 0; i < trash.get_fighters().size(); ++i) {
							if (
								...
								last_fighter = static_cast<Fighter&>(*trash.remove(i));
								found = true;
								break;
							}
						}
		...
		void use_ability(int index = -1) noexcept {
			...
			for (int i = 0; i < effects.size(); ++i) {
				...
				// The ability discards random cards from the deck.
					...
					// The ability discards the player's own cards.
					if (effects[i][1] == SELF_EFFECT) {
						// The number of cards to mill are extracted.
						int mills = std::stoi(effects[i][2]);
...
const DeckCode AGGRO_DECK(
	"Aggro",
	"An aggressive deck featuring Hot Rodder and Monster Trucker.",
	{
		4, // Driver
		2, // Racer
		2, // Hot Rodder
...
const DeckCode CONTROL_DECK(
	"Control",
	"A defensive deck featuring Pirate and Astronaut.",
	{
		4, // Driver
		0, // Racer
		0, // Hot Rodder
...
const DeckCode BLEND_DECK(
	"Blend",
	"A deck featuring all of Driver's rank ups.",
	{
		4, // Driver
		1, // Racer
		1, // Hot Rodder
...
const DeckCode DRIVER_PACKAGE(
	"Driver Package",
	"Contains 4 copies of Driver and all of its rank ups.",
	{
		4, // Driver
		1, // Racer
		1, // Hot Rodder
...
const DeckCode DRIVER_UNLIMITED(
	"Driver Unlimited",
	"Contains 4 copies of Driver and 4 copies of all of its rank ups.",
	{
		4, // Driver
		4, // Racer
		4, // Hot Rodder
...
constexpr int DECK_CODE_COUNT = 6;
...
/* CHANGELOG:
	 ...
	 v0.5.2:
	   Players now draw a life card, at thet start of their turn, when their deck is empty.
