...
constexpr double OPERAND_Y = 0.5;
...
constexpr double TIMER_HEIGHT = 0.9;
constexpr double TIMER_X = 0.95;
constexpr double TIMER_Y = 0.5;
...
class Question {
	public:
		...
		bool question(
			...
			while (!end) {
				...
				while (true) {
					...
					// Pressing the quit key ends the testing.
						...
						back_button.get_rectangle().unclick(Events::LEFT_CLICK)
					...
					else if (!length && Events::unpress(MINUS_KEY)) {
						attempt_string += '-';
						break;
					}
					...
					else if (length && Events::unpress(BACK_KEY)) {
					...
					// The string cannot exceed a specific length.
						...
						for (int i = 0; i < NUMBERS; i++) {
							if (Events::unpress(NUMBER_KEYS[i])) {
								attempt_string += '0' + i;
								pressed = true;
								break;
							}
			...
			if (!finished) {
				// The attempted answer is stoed as an integer.
		...
		Sprite get_sprite(
			...
			operator_.blit(*operators[operation]);
			...
			equals.blit(*operators[QUESTION_TYPES]);
