...
constexpr int EXPLANATION_COUNT = 12;
...
#define EFFECT_CONDITIONS(X) (                            \
	...
	: X == 5 ? static_cast<bool>(INVINCIBILITY_CONDITION) \
	: X == 6 ? static_cast<bool>(CRIPPLE_CONDITION)       \
	: X == 7 ? static_cast<bool>(IMPAIR_CONDITION)        \
	: X == 8 ? static_cast<bool>(FIGHTER_POWER_CONDITION) \
	: X == 9 ? static_cast<bool>(AGILITY_CONDITION)       \
	: X == 10 ? static_cast<bool>(CURSE_CONDITION)        \
...
#define EFFECT_VALUES(X) (         \
	...
	: X == 3 ? COMBO_VALUE         \
	: X == 4 ? FORSAKEN_VALUE      \
	: X == 5 ? INVINCIBILITY_VALUE \
	: X == 6 ? CRIPPLE_VALUE       \
	: X == 7 ? IMPAIR_VALUE        \
	: X == 8 ? FIGHTER_POWER_VALUE \
	: X == 9 ? AGILITY_VALUE       \
	: X == 10 ? CURSE_VALUE        \
	: RANK_BOND_VALUE              \
)
...
#define EFFECT_EXPLANATIONS(X) (         \
	...
	: X == 5 ? INVINCIBILITY_EXPLANATION \
	: X == 6 ? CRIPPLE_EXPLANATION       \
	: X == 7 ? IMPAIR_EXPLANATION        \
	: X == 8 ? FIGHTER_POWER_EXPLANATION \
	: X == 9 ? AGILITY_EXPLANATION       \
	: X == 10 ? CURSE_EXPLANATION        \
...
constexpr int WARLOCK_ABILITY_USES = INFINITE_USES;
...
constexpr int VACUUMANCER_ABILITY_USES = INFINITE_USES;
...
constexpr const char* INNKEEPER_DESCRIPTION =
	"Heal 300 damage from all of your fighters."
;
...
constexpr int FIGHTER_COUNT = 30;
...
const DeckCode MIDRANGE_DECK(
	"Midrange",
	"A offensive deck featuring Monster Trucker, Warlock, Scuba Diver, and Pyrotechnician.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode MILL_DECK(
	"Mill",
	"A mill deck featuring Pyromancer and Lost Soul.",
	{
		0, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
constexpr int DECK_CODE_COUNT = 8;
