...
//{
#define END_DISCARD_CONDITION (effect = effect_search(END_DISCARD_EFFECT))
constexpr const char* END_DISCARD_REPRESENTATION = "End Discard";
#define END_DISCARD_VALUE (*effect)[1]
...
//{
#define INVINCIBILITY_CONDITION effect_search(INVINCIBILITY_EFFECT)
...
constexpr int EXCAVATOR_HEALTH = 1600;
...
constexpr int DIVER_HEALTH = 1600;
...
;
constexpr const char* DIVER_ATTACK_EFFECTS = CLEAR_EFFECT;
constexpr int DIVER_ATTACK_DAMAGE = 400;
...
constexpr const char* CLOUD_SURFER_ABILITY_DESCRIPTION =
	"When this fighter card is played from your hand, draw 3 cards."
;
...
class Fighter: public Card {
	public:
		...
		int damage(int amount) noexcept {
			...
			if (amount < 0 || effect_search(INVINCIBILITY_EFFECT)) {
				amount = 0;
			}
...
class Player: public Affectable {
	public:
		...
		void play(const Fighter& f) noexcept {
			...
			// The fighter has effects when played.
			if (effects) {
				// Energy in play is returned to hand for both players.
				if ((*effects)[1] == ENERGY_BOUNCE_EFFECT) {
					for (int i = 0; i < fighters.size(); ++i) {
				...
				else if ((*effects)[1] == ACTIVE_EFFECT && !index) {
					// The fighter is made invincible.
					if ((*effects)[2] == INVINCIBILITY_EFFECT) {
				...
				else if ((*effects)[1] == HAND_CONVERSION_EFFECT) {
					for (int i = 0; i < hand.get_energy().size(); ++i) {
						hand.get_energy()[i].set_element(fighters[index].get_element());
					}
				...
				// Cards are drawn.
				else if ((*effects)[1] == DRAW_EFFECT) {
					int draws = std::stoi((*effects)[2]);
		...
		void end_discard() noexcept {
			const std::vector<std::string>* effects = effect_search(END_DISCARD_EFFECT);
			
			if (effects) {
				const std::string& name = (*effects)[1];
