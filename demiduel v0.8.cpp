...
constexpr int EXPLANATION_COUNT = 11;
...
#define EFFECT_CONDITIONS(X) (                            \
	...
	: X == 8 ? static_cast<bool>(POWER_CONDITION)         \
	: X == 9 ? static_cast<bool>(AGILITY_CONDITION)       \
	: static_cast<bool>(CURSE_CONDITION)                  \
)
//}
...
constexpr int PLAYER_EXPLANATION_COUNT = 8;
...
#define PLAYER_EFFECT_CONDITIONS(X) (                      \
	...
	: X == 6 ? static_cast<bool>(END_DRAW_CONDITION)       \
...
constexpr const char* WARLOCK_ABILITY_DESCRIPTION =
	...
	"rank in your trash to search your trash for a card to draw."
;
...
constexpr const char* VACUUMANCER_ATTACK_DESCRIPTION =
	...
	"Deal 800 to your opponent's active fighter."
;
constexpr const char* VACUUMANCER_ATTACK_EFFECTS = GUST_EFFECT; // gust
constexpr int VACUUMANCER_ATTACK_DAMAGE = 800;
...
constexpr int DIVER_HEALTH = 1300;
...
constexpr const char* BANSHEE_ATTACK_NAME = "Lost Wail";
...
constexpr int SUPPORTER_COUNT = 31;
			...
			for (int i = 0; i < effects.size(); ++i) {
				...
				// The attack's damage can be increased.
					...
					// The damage is boosted by the number of cards in the void.
						...
						boost += the_void.size() * std::stoi(effects[i][2]);
					}
				}
				...
				// The attack deals damage over time.
					...
					if (effects[i][1] == VOID_EFFECT) {
						std::string curse(
							std::to_string(the_void.size() * std::stoi(effects[i][2]))
						);
...
const DeckCode MIDRANGE_DECK(
	"Midrange",
	"An offensive deck featuring Monster Trucker, Warlock, Pyrotechnician, and Cloud Surfer.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode CONTROL_DECK(
	"Control",
	"A defensive deck featuring Astronaut, Cleric, Excavator, and Diver.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode SPLASH_DECK(
	"Splash",
	"A splash combo deck featuring Pirate, Warlock, Diver, and Cloud Surfer.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode MILL_DECK(
	"Mill",
	"A mill deck featuring Pyrotechnician and Lost Soul.",
	{
		0, // DRIVER
		0, // RACER
		0, // HOT RODDER
...
const DeckCode COMBO_DECK(
	"Combo",
	"A Combo Attack deck featuring Ninja, Samurai, Warlock, and Pirate.",
	{
		1, // DRIVER
		0, // RACER
		0, // HOT RODDER
